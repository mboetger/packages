...
/*package*/ class Translator {
  private static Method getIntroductoryPriceCyclesMethod;
  private static Method isRewardedMethod;
  static {
    try {
      getIntroductoryPriceCyclesMethod = SkuDetails.class.getMethod("getIntroductoryPriceCycles");
    } catch (NoSuchMethodException e) {
      getIntroductoryPriceCyclesMethod = null;
    }
    try {
      isRewardedMethod = SkuDetails.class.getMethod("isRewarded");
    } catch (NoSuchMethodException e) {
      isRewardedMethod = null;
    }
  }
  ...
  static HashMap<String, Object> fromSkuDetail(SkuDetails detail) {
    ...
    String introductoryPriceCycles = null;
    if (getIntroductoryPriceCyclesMethod != null) {
      try {
        Object result = getIntroductoryPriceCyclesMethod.invoke(detail);
        if (result instanceof Integer) {
          introductoryPriceCycles = String.valueOf(result);
        } else if (result instanceof String) {
          introductoryPriceCycles = (String) result;
        }
      } catch (Exception e) {
        // Ignore
      }
    }
    info.put("introductoryPriceCycles", introductoryPriceCycles);
    ...
    if (isRewardedMethod != null) {
      try {
        Object result = isRewardedMethod.invoke(detail);
        if (result instanceof Boolean) {
          isRewarded = (Boolean) result;
        }
      } catch (Exception e) {
        // Ignore
      }
    }
    info.put("isRewarded", isRewarded);
