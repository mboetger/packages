...
abstract class PigeonApiWebView(open val pigeonRegistrar: AndroidWebkitLibraryPigeonProxyApiRegistrar) {
   ...
   */
  abstract fun setNetworkAvailable(pigeon_instance: android.webkit.WebView, networkUp: Boolean)
  ...
  companion object {
    ...
    fun setUpMessageHandlers(binaryMessenger: BinaryMessenger, api: PigeonApiWebView?) {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.webview_flutter_android.WebView.setNetworkAvailable", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val pigeon_instanceArg = args[0] as android.webkit.WebView
            val networkUpArg = args[1] as Boolean
            val wrapped: List<Any?> = try {
              api.setNetworkAvailable(pigeon_instanceArg, networkUpArg)
              listOf(null)
            } catch (exception: Throwable) {
              AndroidWebkitLibraryPigeonUtils.wrapError(exception)
            }
